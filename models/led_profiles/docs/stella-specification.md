...
> The user-approved continuous-shell amendment is implemented as a review
> candidate; neither that arm nor the separately gated removable keeper is
> accepted.
...
| `STELLA-PROFILE-01` | Keep profile retention visually slim, use no drain holes, and make no permanent modification to the aluminium profiles. The profile-following half-round saddle itself must morph continuously into the core-side connector root as one uninterrupted shell; do not add an independently legible beam, rail, rib, tower, brace, newly started extrusion, or sharp form break. Center the keeper fastener lands vertically beneath the future keeper. | User requirement; see [ADR-0001](adr/0001-stella-unmodified-profiles.md). The user first requested a rounded, tapered profile-following transition, then approved the continuous-shell amendment after the separate-support candidate remained visibly segmented. | Retention must be non-invasive, must not load the diffuser or crush the thin wall, and must carry required net section by sizing the shell exterior around the fixed profile cavity rather than by attaching a separate support member. |
...
| Arm and keeper boundary | The authorized arm slice includes the accepted core-side key, two-M3 connection, one continuous profile-derived structural shell, one open profile saddle, arm-side keeper lands and external service clearances. The removable keeper is deferred to the next separately gated slice. | Implement and review the complete arm before defining keeper geometry. | Arm geometry is authorized but remains unaccepted; keeper geometry remains blocked pending arm acceptance. |
