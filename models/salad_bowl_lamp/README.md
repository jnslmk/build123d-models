...
A 20 cm stainless IKEA salad bowl (201 mm as measured), turned over and hung from
a flex through a 42 mm hole drilled in what used to be its bottom, plus one
printed part: a grille of concentric rings on a cross that drops into the
201 mm mouth and is held there by disc magnets (`magnet_count`; the default model
carries none). Nothing is drilled, glued or clamped to the bowl beyond the hole it
already has, and the grille comes out by pulling on it.
...
bowl's own sphere. Above that the band is untouched, so 16.6 of its 22.4 mm are seat
...
## If you plan to hang it on magnets: is your bowl magnetic?
...
where the magnets will sit.

## Then print the fit test

`salad_bowl_lamp.fit_test` is the outer band on its own -- the same
`create_band()` the shade is built from, so it cannot drift from it -- at 29 g
instead of 118 g. It answers the three things that a drawing cannot: where the
...
a segment on purpose: a 2.0 mm arc of PLA flexes far more than any diameter
...
| Item | Qty | Note |
| --- | --- | --- |
| Ø20 cm stainless salad bowl (201 mm measured), 9.5 cm deep | 1 | with a 42 mm hole drilled at the apex |
| Ø5 × 1 mm disc magnet, N42 or similar | optional | `magnet_count` slider -- 0 by default; the fit test takes 8 |
...
in **shear** across the magnets' faces, not in tension pulling them off; what
...
would carry the shade's whole 118 g. Print the fit test before committing to
that: if the
...
worth it, because the first layer is 2.0 mm-wide rings and 200 mm across, and a
...
It is about **118 g and several hours** -- 2.0 mm × 22.4 mm of wall, five rings
and four arms, at 200 mm diameter. Print it in one piece; there is no seam that
would survive being split.

Perimeters matter more than infill here. At a 2.0 mm wall the part is perimeters
all the way through: five of them at 0.4 mm lines, and no infill to speak of.
Behind the magnets a 1 mm pocket in that wall leaves
**1.0 mm** of backing, which is comfortable — it was the binding constraint back
...
1. Glue the magnets into the pockets around the outer band, flush with the
...
one sphere passes through a 201 mm rim circle and touches a plane 95 mm below
it, at R = 100.66 mm. Everything else follows from the inside of that sphere,
...
- below the notch the band keeps a **0.70 mm skirt** — the notch is cut from the
  ...
  letting that skirt fall under `MIN_SKIRT` (0.7 mm -- the library's 0.8 mm
  floor, relaxed for this one wall because it carries no magnets and no load),
  and the skirt's own chamfer is sized to it (0.23 mm, not the part's 0.6 mm —
  two full-size chamfers would swallow it, which is what `checks.py` caught);
- the band's *inner* face is that same sphere 2.0 mm smaller, struck from the
...
- the magnet pockets (when the lamp has any) are bored along that sphere's own
  radii, so each magnet's face is tangent to the steel rather than merely close
  to it, and they sit at mid-band, 11.2 mm up, which is 5.4 mm clear of the
  notch's top;
...
- five rings, evenly spaced 15.5 mm apart, 2.0 mm thick and 22.4 mm tall. The ring
...
`MIN_BACKING` behind every magnet, `MIN_SKIRT` of skirt surviving the notch,
...
The grille sits across the mouth with a **55 mm eye** in the middle
