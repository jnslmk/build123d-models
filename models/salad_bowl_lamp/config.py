...
The floor, not a target, and the default lamp sits exactly on it -- a 2 mm magnet
in a 2.4 mm wall leaves 0.4 mm and no more. It is enough because the backing is
never the loaded part: in service the magnet is pulled *outward* onto the steel
...
class Lamp:
    ...
    def band_outer_radius(self, z: float) -> float:
        ...
        that is the design. The band's outer face *is* ``bowl_inner_radius`` over
        its whole height, which makes this a taper seat rather than a clearance
        fit, and a taper seat is the one kind that cannot jam. The mating
        surfaces converge at 10.5 deg, so a shade printed a few tenths oversize
        ...
        beds down over 20 mm into one that touches at two rims.
        """
...
DEFAULT = Lamp()
"""The lamp this repo built: a 20 cm IKEA bowl, eight 6 x 2 discs, a 2.4 mm wall.
