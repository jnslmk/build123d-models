"""The ALLEN box's three printed parts, one module each.
...
BITS box is cut from as well. What is per-box is the socket layout, the cover
height and the legend, and each module here supplies exactly that.
...
Addressed by name -- ``uv run show drill_storage.hex.allen.base`` -- which is
why the box is a package and not three ``allen_*`` modules: dots are the
