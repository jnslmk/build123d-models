...
| Model | Description |
|-------|-------------|
| [`drill_storage`](models/drill_storage/README.md) | Gridfinity drill holders — one base/cover engine, one holder per tool set (`.wood`, `.metal`, `.hex`), plus a two-material ASA+TPU variant (`.flex`) and a side-by-side comparison scene |
