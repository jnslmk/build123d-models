...
**Edge design for FDM** — *always add chamfers/fillets where appropriate; never ship a part with raw square edges.* The rules below are grounded in the DFM literature (see sources at the end of this subsection); the one-line summary is **chamfer horizontal edges, fillet vertical edges** — the rest is *why* and *when*.

- **Chamfer every exterior horizontal ring** — top and bottom edges that meet the build plate or face up/down. A 45° chamfer (0.5–1.0 mm) is the house default: it prints cleanly (each layer is barely wider than the one below, ≤45° overhang), reads as a deliberate finished edge, and on the **bottom ring doubles as elephant's-foot relief** (the squished first layers get somewhere to bulge into instead of standing proud). Round boxes, lids, trays, plates → chamfer the bottom ring and the top ring, every time.
- **Bottom edges: chamfer, never fillet.** A fillet on a bed-touching edge starts at ~0° (horizontal) and curves up — that near-horizontal start warps/sags and prints rough. If you want a *soft-looking* bottom, use the **chamfer-then-fillet combo**: a 45° chamfer off the bed, then a fillet blended onto the chamfer, so the steepest overhang is still ≤45°.
- **Fillet vertical edges** (the corners running in the print/Z direction). Fillets follow the layer stack cleanly on vertical walls, kill stress risers, and are comfortable to hold. Round the vertical corners of any hand-held or load-bearing part.
- **Fillet internal corners for strength.** A sharp 90° internal corner is a crack initiator; a generous internal radius spreads the load. Do this on brackets, ribs, and anywhere a wall meets a floor under load.
- **Lead-in at every hole/socket/joint mouth** (chamfer or fillet) so mating parts and tools self-guide (see the holes section below).
- **Reliability first**: OCC edge `fillet`/`chamfer` are flaky on non-trivial edge sets (ribbed/hex mouths, rims beside holes) — prefer a **boolean chamfer** (subtract a `Cone`/frustum) there, isolate each edge op with snapshot-restore, and re-query live edges per feature. See the build123d Gotchas section.
- **When to skip**: a purely internal edge that never mates and carries no load, or an edge where a chamfer would eat a thin wall (keep ≥ ~0.4 mm of wall left after the cut) — otherwise, add the edge treatment.

Sources: [BigRep – Fillets vs. Chamfers](https://bigrep.com/posts/fillets-vs-chamfers-in-3d-printed-parts/), [Hydra Research – FFF Design Rules](https://www.hydraresearch3d.com/design-rules), [Xometry – FDM Design Tips](https://xometry.pro/en/articles/fdm-design-tips/), [Protolabs Network – Design for FDM](https://www.hubs.com/knowledge-base/how-design-parts-fdm-3d-printing/).

**Flush lid / stepped-lip (rabbet) joints**: when a lid must close *flush* (no lip standing proud of the body), don't slide the lid over the outside — cut a **rabbet**: recess the top of the box wall inward to a thin lip, and let the lid drop over that lip so the lid's outer face is coplanar with the box body. The wall budget is asymmetric — spend it as `body_wall = lip_wall + clearance + lid_wall` (thick box body, thin lid). Keep `lip_wall ≥ ~0.8 mm` (2 perimeters). Snap beads live *inside* the joint (bead on the lip, matching bead in the lid bore) so they never break the flush exterior; the lid mouth seats **flat-on-flat** on the shoulder ring. See `models/round_snap_box.py`.
